...
import Taro, { getStorageSync as _getStorageSync, setStorageSync as _setStorageSync, getSetting as _getSetting, openSetting as _openSetting } from "@tarojs/taro-h5";
...
export default class Index extends Taro.Component {
  ...
  componentDidMount() {
    const homeData = _getStorageSync('home_data');
    if (homeData) {
      const {
        banner,
        brandList,
  ...
  _getCityName = () => {
    ...
    myMapFunc.getRegeo({
      success: data => {
        ...
        _setStorageSync('location', address);
        this.setState({
          location: address
        });
      },
  ...
  render() {
    ...
    return <View className="home">
        ...
        <Swiper className="swiper" indicatorDots indicatorColor="#ccc" circular autoplay>
          {banner.map(item => {
        ...
        <View className="channel">
          {channel.map(item => {
          return <View className="channel-item" key={item.id} onClick={() => this.toCategoryList(item.id)}>
                <Image className="img" mode="widthFix" src={item.icon_url}></Image>
        ...
        <View className="new-goods">
          ...
          <ScrollView className="goods-list" scrollX>
            {newGoods.map(item => {
            return <View className="list-item" key={item.id}>
                  <Image className="img" src={item.list_pic_url} mode="widthFix"></Image>
        ...
        <View className="topic-list">
          ...
          <ScrollView className="scroll" scrollX>
            {topicList.map(item => {
            return <View className="list-item" key={item.id} onClick={() => this.toDetail(item.id)}>
        ...
        return <View className="new-cate" key={cate.id}>
                <View className="title">{cate.name}好物</View>
                {newCategoryList.length ? <View className="cate-list">
