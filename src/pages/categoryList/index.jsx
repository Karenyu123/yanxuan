        ...
        <ScrollView className="scroll" scrollX scrollLeft={scrollLeft} >
          {
            navData.map((item, index) => {
              return (
                <View
                  key={ item.id }
                  className={ currentNav.id === item.id ? 'nav-item active' : 'nav-item' }
                  onClick={ () => this.navClick(item, index)}
                >{ item.name }</View>
              )
            })
          }
        ...
        <View className="goodslist">
          ...
          </View>
          {
            goodslist.length ?
              <View className="list-wrapper">
                {
                  goodslist.map((item) => {
                    return (
                      <View className="list-item" key={ item.id } onClick={ () => this.toGoods(item.id) }>
              ...
              :
              <View className="null">啊噢，暂无相关商品..</View>
          }
        </View>
      </View>
    )
