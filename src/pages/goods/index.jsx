...
export default class Goods extends Taro.Component {
  ...
  getGoodsDetail = () => {
    ...
    Taro.login({
      success: (res) => {
        if (res.code) {
          Taro.setStorageSync('openid', '0339cGX00ggkNI1taKX00sEQX009cGXn')
          this.setState({
      ...
      <View className="goods-detail">
        ...
        <View className="goods-data">
          <View className="title">商品参数</View>
          {
            attribute.map(item => {
              return (
                <View className="data-item" key={ item.name }>
          ...
          <View className="issues">
            {
              issue.map(item => {
                return (
                  <View className="issue-item" key={item.id}>
