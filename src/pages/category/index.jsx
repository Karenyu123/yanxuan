          ...
          <View className="nav-left">
            {
              listData.map((item, index) => {
                return (
                  <View
                    className={ index === currentIndex ? 'active nav-item' : 'nav-item' }
                    key={ item.id }
                    onClick={ () => this.switchNav(item.id, index) }>
                    { item.name }
                  </View>
                )
              })
            ...
            <View className="list">
              {
                categorylist.map(item => {
                  return (
                    <View className="item" key={ item.id } onClick={() => this.toCategorylist(item.id)}>
